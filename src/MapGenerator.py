...
class TrafficMapGenerator:
    ...
    def __init__(self, allowed_positions_map: np.array, robots_swarm: RobotsSwarm, docking_stations_map: np.array):
                ...
                if robot.failure_detected():  # jeśli robot ma usterkę, to jest pomijany
                    ...
                    if robot.battery_low():  # jeśli robot ma mało baterii
                        ...
                        if self.__docking_stations_map[r_pos[0]][r_pos[1]] > 0:
                            ...
                            direction = self.__goto_nearest_dock(robot.get_id(), robot.get_actual_position())
                            robot.make_move(direction, 0, 0)
                    else:  # jeśli robot jest sprawny
                        direction = self.__generate_allowed_move(robot.get_actual_position())
                        random_load = np.random.randi(-5, 5)
